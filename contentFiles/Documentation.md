...
The `default.tmpl` file also places the contents of the `AppModule.HeaderScript` string into the html header - this can be used to add additional code to the web page for a particular module or method, without having to create a .tmpl file for it.
...
The above functionality, while still working for backward compatinility, has been superceded by the AuthGroup functionality. You can decorate any module or method with AuthGroup attributes. Ticking `Module Permissions` shows a list of all the named AuthGroups, and lets you set the user's access level to each one. By default, they have the same access as the access in their user record, but you can increase or decrease it. The changed access level applies to all methods with a correspondingly named AuthGroup. Be aware that you need to add an AuthGroup to all related methods (e.g. Edit, Data, Save, Delete).
