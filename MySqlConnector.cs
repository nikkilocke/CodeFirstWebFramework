...
namespace CodeFirstWebFramework {
	...
	/// Interface to MySql
	/// </summary>
	public class MySqlConnector : DbInterface {
		...
		public MySqlConnector(Database db, string connectionString) {
			_db = db;
    ...
    ON UPDATE NO ACTION", t.Name, f.ForeignKey.Table.Name, f.Name, f.ForeignKey.Table.PrimaryKey.Name)));
		...
		public Dictionary<string, Table> Tables() {
			...
			string[] restrictions = new string[] { null, schema };
			DataTable tabs = _conn.GetSchema("Tables", restrictions);
			DataTable cols = _conn.GetSchema("Columns", restrictions);
			DataTable fkeyCols = _conn.GetSchema("ReferentialConstraints");
			...
			foreach (DataRow fk in fkeyCols.Select("CONSTRAINT_SCHEMA = " + Quote(schema))) {
			...
			foreach (DataRow table in views.Select("TABLE_SCHEMA = " + Quote(schema))) {
				...
				Field[] fields = new Field[0];/* viewCols.Select(filter, "ORDINAL_POSITION")
		...
		decimal lengthFromColumn(DataRow c) {
			...
			if (type.StartsWith("double")) return 10.4M;
			if (type.StartsWith("bigint")) return 20;
			if (type.StartsWith("int")) return 11;
